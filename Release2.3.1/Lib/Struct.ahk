...
;
; #Include ..\Lib\FileGetVersionInfo.ahk
...
Struct_Put(o,_k="",_v=9223372036854775809,opt="~"){
  If (_v=9223372036854775809){
    ...
    ;~ MsgBox % _v "=" v.Contents "=" StrGet(v.Contents)
    If ((v.Contents && StrGet(v.Contents)="")||v.ContentsInt64=_v){ ;its a value not a string, assume new address
    ...
    v:=Struct("AHKVar",Struct_getVar(_v))
    ;~ MsgBox % _v "=" v.ContentsInt64 "=" StrGet(v.Contents) "?" (StrGet(v.Contents)=""||v.Contents!=_v)
    If ((v.Contents && StrGet(v.Contents)="")||v.ContentsInt64=_v){ ;its a value not a string, assume new address
      NumPut(_v,o[""]+o["`n" _k],(A_PtrSize=8?"UInt64":"UInt"))
    } else {
      o._SetCapacity("`t" _k,(o["`v" _k]="CP0" ? 1 : 2)*StrLen(_v)+2)
...
Struct_getVar(var) {
  static getVarFuncHex:=(A_PtrSize=4)?"8B4C24088B0933C08379080375028B018B4C2404998901895104C3":"488B02837810037507488B00488901C348C70100000000C3"
   ...
   if !(pbin := DllCall("GlobalAlloc","UPTR",0,"uint",StrLen(getVarFuncHex)//2))
    ...
    NumPut(pbin,pFunc+A_PtrSize*(FileGetVersionInfo_AW(A_AhkPath,"FileVersion")>="1.1.00.00"?2:1))
	return pbin
}
