...
Class CChoiceControl Extends CControl ;This class is a ComboBox, ListBox and DropDownList
{
	...
	PostCreate()
	{
		...
		Loop, Parse, Content, % CGUI.GUIList[this.GUINum]._.Delimiter
		...
		if(!(GUI := CGUI.GUIList[this.GUINum]).IsDestroyed)
		{
			...
			if(Name = "SelectedItem")
			{
				Loop % this.Items.MaxIndex()
					if(this.Items[A_Index] = Value)
						GuiControl, % this.GUINum ":Choose", % this.ClassNN, % Gui._.Delimiter A_Index ;Delimiter will trigger g-label
			}
			else if(Name = "SelectedIndex" && Value >= 1 && Value <= this.Items.MaxIndex())
				GuiControl, % this.GUINum ":Choose", % this.ClassNN, % Gui._.Delimiter Value ;Delimiter will trigger g-label
			...
			;~ {
				...
				;~ {
					...
					;~ }
					;~ Loop, Parse, Value, % Gui._.Delimiter
						;~ if(A_LoopField)
				...
				;~ if(!IsObject(Value) && InStr(Value, Gui._.Delimiter Gui._.Delimiter))
				;~ {
	...
	Class CItems
	{
		...
		*/
		Clear()
		{
			...
			GuiControl, % this._.GUINum ":", % Control.hwnd, % Gui._.Delimiter
		...
		Delete(IndexTextOrItem)
		{
			...
			if(IndexTextOrItem > 0 && IndexTextOrItem <= this.MaxIndex())
			{
				...
				if(Selected = IndexTextOrItem)
				{
					...
					GuiControl, % this.GUINum ":Choose", % Control.hwnd, % Gui._.Delimiter Selected ;Select next item. Delimiter will trigger g-label
				}
				else
			...
			*/
			...
			__Set(Name, Params*)
			{
				...
				else if(Name = "Selected" && Value = 1)
				{
					GUI := CGUI.GUIList[this._.GUINum]
					Control := GUI.Controls[this._.hwnd]
					GuiControl, % this._.GUINum ":Choose", % Control.ClassNN, % Gui._.Delimiter this._.Index ;Delimiter will trigger g-label
					return Value
				}
			}
		}
