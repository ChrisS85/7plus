...
BackupPlacesBar()
{
	RegRename("HKCU","HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Policies\comdlg32\Placesbar"," HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Policies\comdlg32\Placesbar7pBackup")
	AddAllButtons(0,1)
}
...
RegRename(root,key,target)
{
	...
	result:=DllCall("Advapi32.dll\RegOpenKeyExA", "uint", hive, "str", key, "uint",0, "uint", 0xF003F, "uint *",hkey)
	if(result=0)
	{
		result:=DllCall("Advapi32.dll\RegCreateKeyExA", "uint", hive, "str", target, "uint", 0, "uint", 0, "uint", 0, "uint", 0xF003F, "uint", 0, "uint *", hNewKey, "uint",0)
		if(result=0)
		{
			result:=DllCall("Advapi32.dll\RegCopyTreeA", uint, hkey, "uint", 0, "uint", hNewKey)
			if(result=0)
			{
