...
WinTraymin(hWnd = "", nFlags = "")
{
	...
	If Not	nFlags
	{
		If Not	((hWnd+=0)||hWnd:=GetForegroundWindow())||((h:=GetWindow(hWnd,4))&&IsWindowVisible(h)&&!hWnd:=h)||!(VarSetCapacity(sClass,15),DllCall("GetClassName","Ptr",hWnd,"str",sClass,"Uint",VarSetCapacity(sClass)+1))||sClass=="Shell_TrayWnd"||sClass=="Progman"
		Return
		OnMessage(MsgNum,"")
		...
		hIcon_%uID% ? "" : (VarSetCapacity(fi,352,0),DllCall("GetWindowThreadProcessId","Ptr",hWnd,"UintP",pid),DllCall("psapi\GetModuleFileNameEx","Ptr",hProc:=DllCall("kernel32\OpenProcess","Uint",0x410,"int",0,"Uint",pid, "Ptr"),"Uint",0,"Uint",&fi+12,"Uint",260),DllCall("kernel32\CloseHandle","Ptr",hProc),DllCall("shell32\SHGetFileInfo","Uint",&fi+12,"Uint",0,"Uint",&fi,"Uint",352,"Uint",0x101),hIcon_%uID%:=NumGet(fi)),DllCall("GetWindowText","Ptr",hWnd,"Uint",NumPut(hIcon_%uID%,NumPut(nMsg,NumPut(1|2|4,NumPut(uID,ni,8)))),"int",64)
		Return	hWnd_%uID%:=DllCall("shell32\Shell_NotifyIcon","Ptr",hWnd_%uID% ? 1 : 0,"Uint",&ni) ? hWnd : DllCall("ShowWindow","Ptr",hWnd,"int",5)*0
	}
	Else If	nFlags > 0
	{
		...
		Return	uID ? (hWnd_%uID% ? (DllCall("shell32\Shell_NotifyIcon","Uint",2,"Uint",NumPut(uID,ni,8)-12),hWnd_%uID%:="") : "",nFlags==2&&hIcon_%uID% ? (DllCall("DestroyIcon","Ptr",hIcon_%uID%),hIcon_%uID%:="") : "") : ""
	}
	Else
		Loop, % nIcons
			hWnd_%A_Index% ? (DllCall("shell32\Shell_NotifyIcon","Uint",2,"Uint",NumPut(A_Index,ni,8)-12),DllCall("ShowWindow","Ptr",hWnd_%A_Index%,"int",5),hWnd_%A_Index%:="") : "",hIcon_%A_Index% ? (DllCall("DestroyIcon","Ptr",hIcon_%A_Index%),hIcon_%A_Index%:="") : ""
}
